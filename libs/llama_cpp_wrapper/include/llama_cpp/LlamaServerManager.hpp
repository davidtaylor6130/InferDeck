...
public:
    ...
    bool Start(const std::string& model_path, int gpu_layers = -1, int context_size = 8192, int port = 18080);
    void Stop();
    bool IsRunning() const;
    int GetPort() const;
    bool WaitForReady(int timeout_seconds = 120) const;
    bool Restart(const std::string& new_model_path, int gpu_layers = -1, int context_size = 8192);
