...
import { DASHBOARD_URL, LLAMA_BACKEND } from '../utils';

export const ServicesPage: React.FC<PageProps> = ({ state, actions }) => {
  const services = normalizeServices(state.servicesList);
  return (
    <SectionCard title="Service Control" eyebrow="Gateway, llama.cpp, and worker health">
      <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
        {services.map(service => <ServiceStatusCard key={service.id} service={service} onStart={actions.startService} onStop={actions.stopService} onRestart={(id) => id === 'llama-server' ? actions.restartBackend() : actions.restartService(id)} onCopied={actions.toast} />)}
      </div>
    </SectionCard>
  );
};
...
function normalizeServices(services: ServiceRecord[]): ServiceRecord[] {
  const fallback: ServiceRecord[] = [
    ...
    { id: 'llama-server', name: 'llama.cpp', kind: 'llama_cpp', status: 'running', pid: null, baseUrl: LLAMA_BACKEND, lastHealthcheckAt: new Date().toISOString() },
