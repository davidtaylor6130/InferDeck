...
test('official OpenAI SDK parses InferDeck Chat streaming usage ordering', async () => {
  ...
  assert.equal(chunks[2].choices[0].delta.tool_calls[0].function.name, 'lookup');
  ...
  assert.equal(chunks[4].usage, null);
  assert.deepEqual(chunks[5].choices, []);
  assert.equal(chunks[5].usage.prompt_tokens, 8);
  assert.equal(chunks[5].usage.prompt_tokens_details.cached_tokens, 3);
  assert.equal(chunks[5].usage.completion_tokens, 12);
  assert.equal(chunks[5].usage.total_tokens, 20);
});
