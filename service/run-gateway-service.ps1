...
while ($true) {
    ...
    try {
        $proc = Start-Process -FilePath "$exe" -ArgumentList "$args" -WorkingDirectory "$workDir" -NoNewWindow -PassThru -RedirectStandardOutput "$logDir\stdout.log" -RedirectStandardError "$logDir\stderr.log"
        $proc | Wait-Process
